...
def is_win_win_solution(vector_a, vector_b, distribution_a, distribution_b):
...
def is_parto(oritinal_eval_a, new_eval_a, original_eval_b, new_eval_b):
...
def pareto_improvement(vector_a, vector_b, distribution_a, distribution_b):
    ...
    while not found and count < 10000:
        new_distribution_a, new_distribution_b = generate_distributions()
        new_eval_a = eval(vector_a, new_distribution_a)
        new_eval_b = eval(vector_b, new_distribution_b)
        if is_parto(oritinal_eval_a, new_eval_a, original_eval_b, new_eval_b):
            found = True
